...
layout (rgba32f, binding = 0) coherent uniform image2D sum_aici_k;
layout (rgba32f, binding = 1) coherent uniform image2D sum_ai_k;
layout (rgba32f, binding = 2) coherent uniform image2D prod_1_ai_k;
...
void main() 
{
   ...
   //project intersection point to screen (NDC)
    ...
    //Depth peeling: PASS i 
    beginInvocationInterlockNV();

        vec2 coords = vec2(gl_FragCoord.xy/viewportSize); 
        ivec2 icoords = ivec2(gl_FragCoord.xy);
        float previousDepth = texture(previousDepthTexture, coords).r;//retrieve nearest depth so far
        float frag_count = imageLoad(sum_aici_k, icoords).w;
        if (newPos.z < previousDepth && frag_count <= 9) //pass if you are in the first K layers find sums and products. Do not update if fragment count exceeds 10
        { 
            //Σaici
            vec4 prev_val = imageLoad(sum_aici_k, icoords);
            prev_val.xyz += color*a;
            prev_val.w += 1;
            imageStore(sum_aici_k, icoords, prev_val);

            //Σai
            prev_val = imageLoad(sum_ai_k, icoords);
            prev_val.xyz += vec3(a);
            prev_val.w += 1;
            imageStore(sum_ai_k, icoords, prev_val);

            //Π(1 - ai)
            prev_val = imageLoad(prod_1_ai_k, icoords);
            prev_val.xyz *= (1 - a);
            prev_val.w += 1;
            imageStore(prod_1_ai_k, icoords, prev_val);

            gl_FragDepth = newPos.z; //write to depthbuffer
